        ...
        if (e.getSource().equals(ok_bn)) {
            ...
            for (int i = 0; i <= ParticipantModal.arrlist.toArray().length - 1; i++) {
                if (ParticipantModal.arrlist.get(i).getMeeting_name().equals(comboBox.getSelectedItem().toString())) {
                    ...
                    OrganizerTable.dataTable.setValueAt(ParticipantModal.arrlist.get(i).getDate(), num_row, num_colum++);
                    OrganizerTable.dataTable.setValueAt(ParticipantModal.arrlist.get(i).getTime(), num_row, num_colum);
                    num_colum = 0;
                    num_row++;
                }
            }
